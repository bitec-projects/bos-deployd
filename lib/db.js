...
function getConnection(db) {
	...
	return new Promise(function (resolve, reject) {
		try {
			MongoClient.connect(db.connectionString, db.connectionOptions, function (err, client) {
				if (!err) {
					db.Db = client.db(db.options.name);
					resolve(db.Db);
				} else {
					// log error
...
function collection(store, fn) {
	...
	return getConnection(db)
		.then(function (mdb) {
			return new Promise(function (resolve, reject) {
				let _collection = mdb.collection(store.namespace);
				if (fn) fn(null, _collection);
				resolve(_collection);
			});
		})
		.catch(function (err) {
...
Store.prototype.insert = function (object, fn) {
	...
	collection(this, function (err, col) {
		...
		col.insertOne(object, function (err, result) {
			...
			col.findOne({ _id: result.insertedId }, function (err, result) {
				if (err) {
					fn(err);
					return;
				}
...
Store.prototype.find = function (query, fn) {
	...
	if (!_.isObject(fields)) {
		...
		Object.assign(options, { projection: fields });
	}
	...
	collection(this, function (err, col) {
		...
		col.update(
			...
			function (err, r) {
				...
				store.identify(query);
				fn(err, r ? { count: r.modifiedCount } : null);
			},
			multi
		);
	});
